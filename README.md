...
| **5** | ✅ done | **admin**         | _(uses 0001/0003)_   | Admin dashboard: Listings/Users tabs; all listings + filter by user (tap → detail); **minimal** user mgmt (list users, change role). _Soft-delete/ban deferred to step 6 (needs `deleted_at`); listing status mgmt in step 7._ | Admin browses all listings, filters by user, changes a user's role |
| **6** | ✅ done | **profile**       | `0005_profiles_ext`  | Added `first_name,last_name,age,deleted_at` + age≥18 check + trigger copies signup metadata; editable Profile screen w/ **soft delete**; **Reactivate** gate on re-login; registration first/last name + age≥18 + show/hide password (Field `toggleSecure`); admin **ban/unban** + deactivated badge | User edits info; soft-deletes (signed out); re-login offers reactivation; admin bans/unbans |
| **7** | ✅ done | **listings mgmt** | _(uses 0003)_        | Merchant: status filter chips on own listings; shared AnnouncementForm; **Edit** screen (active only, owner/admin); **archive**→`inactive` via Detail; admin status mgmt (deferred from step 5); Detail refetches on focus | Merchant filters, edits an active listing, archives one; admin manages any listing's status |
| **8** | ✅ done | _RLS hardening_   | `0006_rls_policies`  | RLS enabled on profiles/announcements/app_settings; `user_is_active()` helper; public/anon read of active listings (active owners only) + settings; owner+admin writes; no self-escalation to admin; verified via anon-role tests (guest sees active only, 0 profiles; soft-deleted owner hides listings) | Guest reads active only; soft-deleted hidden; each role limited to its matrix |
| **9** | ✅ done | _Polish_          | —                    | pull-to-refresh on listings + admin lists; loading/error/empty states verified across screens; setup guide + `.env.example` finalized; fixed stale migration header comments | MVP demo-ready end-to-end                                                            |
| **10**| ✅ done | **moderation**    | `0007`,`0008`        | New listings start `pending` (admin approve→`active` / reject→`rejected`); server-side `enforce_moderation` trigger blocks self-approval; admin status filter + Detail approve/reject; merchant resubmit; status pills/filters extended | Merchant submits → pending; admin approves to publish; merchants cannot self-approve (DB-enforced) |
| **11**| ✅ done | **product fields**| `0009`               | Optional French title/description + quantity (`0009`); sectioned bilingual add/edit form; localized display (FR when UI=fr); editing an approved listing requeues it for review; richer My Products header/banner/empty state | Merchant adds AR + optional FR content & quantity; FR users see translated cards; edits re-enter review |
